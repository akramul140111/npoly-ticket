@section('content')
    ...
    @php $actionUrl=url('/updateTicketDetails'); @endphp
    <script>$('form').parsley();</script>
    <?php ini_set('memory_limit', -1) ?>
    <div class="flash-message"></div>
    <div class="x_content">
        <form id="projectSetupForm" data-parsley-validate="" role="form" method="post" action="{{url('/updateTicketDetails/'.$id)}}"
                ...
                <div class="col-md-6 col-sm-6">
                    <select class=" form-control ticket-status" name="ticket_status" id="" style="width: 500px !important;">
                        ...
                        @foreach($ticketStatus as $status)
                            <option value="{{$status->lookup_data_id}}">{{$status->lookup_data_name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
